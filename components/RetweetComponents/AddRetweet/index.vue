...
<template>
    <div class="flex flex-col gap-10 px-5 border border-t-gray-200 border-t-2 border-b-gray-200 border-b-2 py-5">
    <div class="image_input flex  gap-5 ">
        ...
        <div class="self-center basis-9/12 ">
            <textarea v-model="text_field" class="border-none outline-none w-full" type="text" placeholder="Tweet your reply" />
        </div>
    </div>
    <div v-if="imageURL.length != 0">
        <img class="w-full rounded-lg" :src="imageURL" />
    </div>
    <div class="media_buttons pl-5 flex justify-between">
...
formData.append('replyTo',param.params.retweets);
formData.append('image', imageuploader.value);
  ...
  if(posted == true){
    ...
    tweetStore.getTweetById(param.params.retweets)
